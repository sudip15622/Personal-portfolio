    ...
    <main>
      <section className="flex items-center justify-between">
        ...
        >
          ...
          <p className="w-full max-w-[600px] text-[1.1rem] text-[var(--sec-text)] my-3">
      ...
      <section className="relative flex justify-between items-start">
        <div className="flex flex-col gap-5">
          <h2 className='font-bold text-[3.5rem]'>My Skills</h2>
          <p className='w-full max-w-[500px] text-[1.1rem] text-[var(--sec-text)]'>I have honed my skills across multiple domains, from building dynamic web applications to exploring AI/ML and creative design. Here are the key areas I specialize in:</p>
          <motion.ul className="w-full max-w-[500px] flex flex-col mt-3"
                ...
                <React.Fragment key={index}>
                  <li className='flex gap-5 w-full'>
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <p className='w-full max-w-[350px] text-[1.1rem] text-[var(--sec-text)]'>{skill.description}</p>
                    </div>
                  </li>
        ...
        <div className='flex flex-col gap-4 max-w-[500px] mt-10'>
          ...
          <motion.div className="flex flex-wrap items-center gap-x-5 gap-y-7"
            initial={{ opacity: 0, y: 50 }}
            whileInView={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6, ease: "easeOut" }}
            viewport={{ once: true, amount: 0.2 }}
          >
            {techNames.map((techName, index) => {
              return (
                <div key={index} className='flex flex-col items-center justify-center text-center gap-2 w-[80px]'>
      ...
      <section className="relative container mx-auto">
        <h1 className="text-[3.5rem] font-bold text-center mb-12">My Journey & Experiences</h1>
        <div className="relative mx-auto w-full">
          {/* Vertical Line */}
          <div className="absolute left-1/2 transform -translate-x-1 bg-[var(--border-color)] w-1 h-full"></div>
          ...
          <div className="flex flex-col space-y-12">
            {journeyData.map((item, index) => {
              return (
                ...
                >
                  {/* Icon */}
                  <div className="absolute left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-[var(--action-color)] p-3 rounded-full text-2xl">
                    {item.icon}
                  </div>

                  <motion.div className="flex flex-col gap-3 w-5/12 p-4 bg-[var(--bg-sec-color)] rounded-lg shadow-lg"
